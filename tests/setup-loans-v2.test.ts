...
import { registerAttestors, registerContract } from "./lib/dlc-helper";
...
describe("loan setup tests", () => {
  ...
  it("ensure loan can't be locked by random sender", () => {
    ...
    const p:ParsedTransactionResult = simnet.callPublicFn(CONFIG.VITE_DLC_UASU_LOAN_CONTRACT.split('.')[1], "set-status-funded", functionArgs, sender);
