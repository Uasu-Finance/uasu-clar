...
(define-public (borrow (loan-id uint) (amount uint))
  (let (
    ...
    (unwrap! (ok (contract-call? 'ST1JHQ5GPQT249ZWG6V4AWETQW5DYA5RHJB0JSMQ3.dlc-stablecoin transfer amount sample-protocol-contract (get owner loan) none)) err-stablecoin-issue-failed)
  )
)
...
(define-public (repay (loan-id uint) (amount uint))
  (let (
    ...
    (map-set loans loan-id (merge loan { vault-loan: (- vault-loan-amount amount) }))
    (unwrap! (ok (contract-call? 'ST1JHQ5GPQT249ZWG6V4AWETQW5DYA5RHJB0JSMQ3.dlc-stablecoin transfer amount (get owner loan) sample-protocol-contract none)) err-stablecoin-repay-failed)
  )
)
