...
(define-read-only (calc-principal-repay (repaying uint) (principal uint) (start-block uint) (rate uint) (interest-total uint))
  (let (
      (period (- burn-block-height start-block))
      (shifted-princ   (shift-value principal ten-to-power-12))
      (shifted-repayment   (shift-value repaying ten-to-power-12))
      (shifted-interest (/ (shift-value (/ (* principal rate period) u100) ten-to-power-12) interest-period))
      (new-princ (- shifted-princ (- shifted-repayment (+ shifted-interest  (shift-value interest-total ten-to-power-12)))))
    )
    (unshift-value new-princ ten-to-power-12)
  )
)
...
(define-public (repay (loan-id uint) (amount uint))
  (let (
      ...
      (new-principal (calc-principal-repay amount vault-loan-amount start-block interest-rate interest-total))
    )
    (asserts! (is-eq (get owner loan) tx-sender) err-unauthorised)
    (asserts! (is-eq (get status loan) status-funded) err-dlc-not-funded)
