...
(define-public (borrow (loan-id uint) (amount uint))
  (let (
      ...
      (new-principal (calc-principal false vault-loan-amount start-height interest-rate))
      (potential-fee (calc-liquidation-fee collateral liquidation-fee))
    )
    (asserts! (is-eq (get owner loan) tx-sender) err-unauthorised)
    (asserts! (is-eq (get status loan) status-funded) err-dlc-not-funded)
    (asserts! (> collateral (+ amount vault-loan-amount potential-fee interest-total)) err-borrow-limit-reached)
    (map-set loans loan-id (merge loan { vault-loan: new-principal, interest-start-height:  burn-block-height }))
...
(define-read-only (calc-principal (repaying bool) (amount uint) (start-block uint) (rate uint))
  (let (
      (period (- burn-block-height start-block))
      (shifted-amount   (shift-value amount ten-to-power-12))
      (shifted-interest (/ (shift-value (/ (* amount rate period) u100) ten-to-power-12) interest-period))
    )
    ;;(+ interest-total (/ (* period interest) interest-period))
    (if repaying
      (unshift-value (- shifted-amount shifted-interest) ten-to-power-12)
      (unshift-value (+ shifted-amount shifted-interest) ten-to-power-12)
    )
  )
)
...
(define-read-only (calc-liquidation-fee (collateral uint) (fee uint))
  (/ (* collateral fee) u10000)
)
...
(define-public (repay (loan-id uint) (amount uint))
  (let (
      ...
      (interest-rate (get interest-rate loan))
      (start-height (get interest-start-height loan))
      (new-principal (calc-principal true vault-loan-amount start-height interest-rate))
    )
    (asserts! (is-eq (get owner loan) tx-sender) err-unauthorised)
    (asserts! (is-eq (get status loan) status-funded) err-dlc-not-funded)
